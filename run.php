...
class Bot extends Functions{
	...
	private function getExt(){
		...
		return json_decode($this->curl(host."extn/get/", $this->headers(), $data)[1],1);
	}
	private function ExtPopup($hash){
		$data = "hash=".$hash."&popup=submit";
		return json_decode($this->curl(host."extn/popup/", $this->headers(), $data)[1],1);
	}
	private function ClaimPopup($hash){
		$data = "hash=".$hash;
		return json_decode($this->curl(host."extn/popup-check/", $this->headers(), $data)[1],1);
	}
	private function ExtTeas($hash){
		$data = "hash=".$hash;
		return json_decode($this->curl(host."extn/status/", $this->headers(), $data)[1],1);
	}
